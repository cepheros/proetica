...
class Functions_Tickets{
	...
	public static function gerPassword(){
	    $tickets = new Functions_Tickets;
	    return $tickets->gerSenha(6);	    
	}
	...
	private function gerSenha($qtd = 6){
		$newpassword = '';
		$CaracteresAceitos = 'ABCDEFGHIJKLMNOPQRSTUVXYZ1234567890';
		$max = strlen($CaracteresAceitos)-1;
		$password = null;
		for($i=0; $i < $qtd; $i++) {
			$newpassword .= $CaracteresAceitos{mt_rand(0, $max)};
	...
	public function  sendMailProspect($ticket,$tipo = 'novo'){
		$db = new Crm_Model_Prospects_Basico();
		$dadosticket = $db->fetchRow("id_registro = '$ticket'");
	
		$id_messages = Crm_Model_Marketing_Campanhas::getMesssages($dadosticket->campanhaproposta);
		
		$campanha = Crm_Model_Marketing_Campanhas::getDataCampanha($dadosticket->campanhaproposta);
	        if($tipo == 'novo'){   	
			    $mensagem = System_Model_MensagensSistema::getMensagem($id_messages['new']);
	        }else{
	            $mensagem = System_Model_MensagensSistema::getMensagem($id_messages['reply']);
	        }
			$assunto = $campanha->assuntomensagem;
		
		$sendmessage = $this->montaMensagem($mensagem->htmlmensagem, $dadosticket,$dadosreply);
		$empresa = System_Model_Empresas::getNomeFantasiaEmpresa($campanha->id_empresa);
		//$departamento = Crm_Model_TicketsDeptos::getNomeDepto($dadosticket->departamento);
	
		//$datadepto = Crm_Model_TicketsDeptos::getDeptoConfigs($dadosticket->departamento);
	
		$assuntomessage = "{$assunto} ";
	
		$dados['messagehtml'] = $sendmessage;
		$dados['messagetxt'] = strip_tags($sendmessage);
		$dados['formemail'] = $datadepto->deptoemail;
		$dados['fromname'] = $empresa;
		$dados['messageto'] = $dadosticket->emailsolicitante;
		$dados['messagetoname'] = $dadosticket->nomesolicitante;
		$dados['messagesubject'] = $assuntomessage;
		$this->sendMessage($dados);
