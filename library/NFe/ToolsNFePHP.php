...
class NFe_ToolsNFePHP {
   ...
   /**
    ...
    function __construct($aConfig='',$mododebug=2,$exceptions=false) {
        ...
        $this->xsdDir = NFeCONFIGS_PATH . DIRECTORY_SEPARATOR . 'schemes'. DIRECTORY_SEPARATOR;
        //carrega o caminho para os certificados
        $this->certsDir =  APPLICATION_PATH. '/data/certs/' . $this->idEMP . DIRECTORY_SEPARATOR;
        ...
        $this->aURL = $this->loadSEFAZ(NFeCONFIGS_PATH . DIRECTORY_SEPARATOR . $this->xmlURLfile,$this->tpAmb,$this->UF);
    ...
    */
    ...
    public function statusServico($UF='',$tpAmb='',$modSOAP='2', &$aRetorno=''){
        try {
            ...
            if (!$this->enableSCAN){
                $aURL = $this->loadSEFAZ( NFeCONFIGS_PATH . DIRECTORY_SEPARATOR . $this->xmlURLfile,$tpAmb,$UF);
            } else {
                $aURL = $this->loadSEFAZ( NFeCONFIGS_PATH . DIRECTORY_SEPARATOR . $this->xmlURLfile,$tpAmb,'SCAN');
            }
    ...
    public function getListNFe($AN=false,$indNFe='0',$indEmi='0',$ultNSU='',$tpAmb='',$modSOAP='2',&$resp=''){
        try {
            ...
            if (!$AN){
                $aURL = $this->loadSEFAZ(NFeCONFIGS_PATH . DIRECTORY_SEPARATOR . $this->xmlURLfile,$tpAmb,$this->UF);
                $sigla = $this->UF;
            } else {
                $aURL = $this->loadSEFAZ( NFeCONFIGS_PATH . DIRECTORY_SEPARATOR . $this->xmlURLfile,$tpAmb,'AN');            
                $sigla = 'AN';
            }    
            if($ultNSU == ''){
    ...
    */
    ...
    private function __getUltNSU($sigla='',$tpAmb=''){
        try {
            ...
            $nsufile = NFeCONFIGS_PATH. '/numNSU.xml';
            if (!is_file($nsufile)){
    ...
    private function __trata239($xml='',$UF='',$tpAmb='',$servico='',$versaodefault=''){
        ...
        try {
            ...
            if (($cStat == '239' || $cStat == '238') && $versao != $versaodefault){
                ...
                $nfews = NFeCONFIGS_PATH . '/' . $this->xmlURLfile;
                if (is_file($nfews)){
