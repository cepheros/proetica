<?php
class Cadastros_ClientesController extends Zend_Controller_Action{
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				...
				try{
					...
					$this->log->log("Cadastro do Cliente {$formData['razaosocial']} alterado por: {$userInfo->nomecompleto}",Zend_Log::INFO);
					$this->_redirect("cadastros/clientes/cadastro/id/$id");
				}
				catch (Exception $e){
	...
	public function outrosdadosAction(){
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				...
				$this->log->log("Dados Adicionais do cliente {$formData['id_pessoa']} foram alterados por: {$userInfo->nomecompleto}",Zend_Log::INFO);
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				...
				echo json_encode($formData);				
					...
					$formData['tipoendereco'] = System_Model_Tiposenderecos::getNomeEndereco($formData['tipoendereco']);
					$this->log->log("Dados de endereco do cliente {$formData['id_pessoa']} foram adicionados por: {$userInfo->nomecompleto}",Zend_Log::INFO);
					echo json_encode($formData);
				}
				}
				catch (Exception $e){
